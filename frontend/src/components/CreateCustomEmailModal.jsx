...
const CreateCustomEmailModal = ({ availableDomains, onClose, onSuccess }) => {
  ...
  // ── Measure the real bottom bar height so spacing is always correct ──
  const [bottomBarHeight, setBottomBarHeight] = useState(0);

  useEffect(() => {
    const measure = () => {
      // BottomBar renders a fixed element with classes "fixed bottom-0 ... lg:hidden"
      // We grab it by querying any fixed bottom-0 bar that's visible (not the modal itself)
      const bar = document.querySelector('.fixed.bottom-0.left-0.right-0.bg-white.border-t');
      if (bar && window.getComputedStyle(bar).display !== 'none') {
        setBottomBarHeight(bar.getBoundingClientRect().height);
      } else {
        setBottomBarHeight(0); // desktop / lg screens, no bottom bar
      }
    };
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-t-xl sm:rounded-xl shadow-xl w-full max-w-md overflow-y-auto flex flex-col"
        style={{ maxHeight: `calc(90vh - ${bottomBarHeight}px)` }}
      >
