...
const Inbox = () => {
  ...
  const groupedBySender = useMemo(() => {
    ...
    const groups = Array.from(map.entries()).map(([senderEmail, mails]) => {
      ...
      const hasAttachment = sorted.some((m) => m.attachments && m.attachments.length > 0);
      return { senderEmail, emails: sorted, latest: sorted[0], unreadCount, isStarred, hasAttachment };
    });
    groups.sort(
            ...
            groupedBySender.map((group) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center px-3 py-2.5 border-b border-gray-100 active:bg-gray-50 transition-colors cursor-pointer ${isSelected ? 'bg-purple-50' : ''}`}
                >
                  {/* Avatar */}
