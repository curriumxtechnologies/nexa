...
const Sent = () => {
  ...
  const groupedByRecipient = useMemo(() => {
    ...
    const groups = Array.from(map.entries()).map(([recipientEmail, mails]) => {
      ...
      return { recipientEmail, emails: sorted, latest: sorted[0], hasAttachment };
    });
    groups.sort(
      (a, b) => new Date(b.latest.sentAt || b.latest.createdAt) - new Date(a.latest.sentAt || a.latest.createdAt)
    );
    return groups;
  ...
  const handleThreadClick = (group) => navigate(`/email/${group.latest.emailId}`);
  const handleEmailClick = (email) => navigate(`/email/${email.emailId}`);
                ...
                {showFilterMenu && (
                  <div className="absolute right-0 top-full mt-2 w-40 bg-white rounded-lg shadow-lg border border-gray-100 z-20">
                    <div className="py-1">
                      ...
                      <button onClick={() => { setFilterType('delivered'); setShowFilterMenu(false); }} className={`w-full text-left px-3 py-2 text-xs ${filterType === 'delivered' ? 'bg-purple-50 text-purple-600' : 'text-gray-600'}`}>
                        Delivered
                      </button>
                      <button onClick={() => { setFilterType('failed'); setShowFilterMenu(false); }} className={`w-full text-left px-3 py-2 text-xs ${filterType === 'failed' ? 'bg-purple-50 text-purple-600' : 'text-gray-600'}`}>
                        Failed
                      </button>
